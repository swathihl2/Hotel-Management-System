...
<dir><h2><center>
	<table class="login">
		<thead><th><h4 style="color:skyblue;">Registration form for Customers</h4></th></thead>
		<td><center>
			<form method="POST" action="LogupC.php">
		<label >User Name</label> <input type="text" name="name"required><br>
		<label>Address</label><input type="text" name="Address"required><br>
		<label>Phone</label><input type="number" length="10" name="Phone"required><br>
       ...
       <label>Confirm Password</label>      <input name="cpassword"  type="password" onChange="onChange()" /> </label>  <br />
		...
		<button onclick="window.location.href='indexC.php'">Login</button>&nbsp
